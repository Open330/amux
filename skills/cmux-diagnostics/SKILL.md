...
3. Hook installation:
   ...
   amux hooks setup --agent opencode
   amux hooks setup
   ```

   Only run install or uninstall commands after the user agrees. `amux hooks setup` installs supported agents found on PATH and skips missing agents.
...
- Prefer narrow fixes such as `amux hooks setup --agent codex` over reinstalling every integration.
