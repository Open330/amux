...
`amux browser` currently does not expose a built-in video recording command.

Why: amux browser automation runs on WKWebView, and the agent-browser style recording pipeline is Chrome/CDP-specific.
...
```bash
amux browser surface:7 screenshot > /tmp/step1.b64
amux browser surface:7 click e3 --snapshot-after --json
amux browser surface:7 screenshot > /tmp/step2.b64
```

### 2. Snapshot Timeline

```bash
amux browser surface:7 snapshot --interactive > /tmp/snap-1.txt
amux browser surface:7 click e3 --snapshot-after --json > /tmp/action-1.json
amux browser surface:7 snapshot --interactive > /tmp/snap-2.txt
```
