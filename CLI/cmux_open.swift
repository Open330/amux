...
enum CMUXAgentTurnDiffBaselineFile {
    static func path(env: [String: String] = ProcessInfo.processInfo.environment) -> String {
        ...
        return homeExpandedPath("~/.amux/agent-turn-diff-baselines.json", env: env)
    }
...
extension CMUXCLI {
    ...
    private func taggedDiffViewerExecutableURL(socketPath: String) -> URL? {
        ...
        for name in ["amux", "cmux"] {
            let candidate = binURL.appendingPathComponent(name, isDirectory: false).standardizedFileURL
    ...
    func diffSubcommandUsage() -> String {
        ...
        Options:
          ...
          --layout <split|unified>     Diff layout (default: unified; configurable via diffViewer.defaultLayout in amux.json)
