...
if [ "$APP_BASENAME" = "amux DEV.app" ] && [ "${CMUX_ALLOW_UNTAGGED_CA_REGRESSION:-0}" != "1" ]; then
  echo "ERROR: refusing to launch untagged amux DEV.app without CMUX_ALLOW_UNTAGGED_CA_REGRESSION=1" >&2
  exit 2
fi

BINARY="$APP_PATH/Contents/MacOS/amux DEV"
if [ ! -x "$BINARY" ]; then
