...
#   - Skip any tag whose `amux DEV <tag>` app is currently running.
...
# Running amux DEV processes by tag (the app name embeds the tag).
running_tags=()
while IFS= read -r line; do
    # Match "amux DEV <tag>" (with or without .app suffix).
    if [[ "$line" =~ cmux\ DEV\ ([A-Za-z0-9._-]+) ]]; then
        running_tags+=("${BASH_REMATCH[1]}")
    fi
done < <(pgrep -fl "amux DEV " 2>/dev/null || true)
