...
cmux_attach_mac_bundle_id() {
  printf 'com.open330.amux.debug.%s' "$(cmux_attach__bundle_seg "$1")"
}
...
# sanitized slug, matching reload.sh (`APP_NAME="amux DEV ${TAG_SLUG}"`); the raw
# tag would miss for any tag whose slug differs (e.g. "Fix Foo" -> "fix-foo").
cmux_attach_mac_app_path() {
  local slug
  slug="$(cmux_attach__slug "$1")"
  printf '%s/Library/Developer/Xcode/DerivedData/cmux-%s/Build/Products/Debug/amux DEV %s.app' \
    "$HOME" "$slug" "$slug"
}
...
cmux_attach_ensure_mac() {
  ...
  if [[ -S "$sock" ]]; then
    ...
    pkill -f "amux DEV ${slug}.app/Contents/MacOS/amux DEV" 2>/dev/null || true
