...
kill_stale_app_host() {
  # Kill app-host executables (matched by their .../Build/Products/.../amux DEV
  ...
  [ -n "$ci_app_host_root" ] && \
    pkill -f "${ci_app_host_root%/}/.*Build/Products/.*amux DEV" 2>/dev/null || true
}
...
while [ "$attempt" -le "$max_attempts" ]; do
  ...
  : >"$log_path"
  # Self-hosted macOS runners reuse the GUI session. A stale "amux DEV" app-host
