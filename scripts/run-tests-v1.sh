...
APP="$DERIVED_DATA_PATH/Build/Products/Debug/amux DEV.app"
RUN_TAG="tests-v1"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: amux DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "amux DEV" || true
  pkill -x "cmux" || true
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "amux DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.open330.amux.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  CMUX_TAG="$RUN_TAG" CMUX_UI_TEST_MODE=1 "$APP/Contents/MacOS/amux DEV" >/dev/null 2>&1 &
