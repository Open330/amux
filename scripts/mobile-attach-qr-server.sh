...
def tagged_app_executable() -> str:
    return os.path.join(tagged_app_path(), "Contents", "MacOS", "amux DEV")


def app_info() -> dict:
    """Single source of truth for what 'Open amux DEV <tag>' actually does.
    ...
    try:
        out = subprocess.run(
            ["pgrep", "-f", f"amux DEV {TAG}.app/Contents/MacOS/amux DEV"],
            check=False,
            timeout=2,
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _serve_qr_page(self) -> None:
        ...
        with open(html_path, "rb") as fh:
            html = fh.read()
        # Inject a meta refresh + a small banner + an "Open amux DEV <tag>"
        ...
        banner = (
            ...
            + b'      btn.innerHTML="Open <code>amux DEV "+info.tag+"</code> \\u00b7 built "+built+running;\n'
            + b'      btn.title=info.app_path;\n'
            + b'    }else{\n'
            + b'      btn.disabled=true;\n'
            + b'      btn.innerHTML="<code>amux DEV "+info.tag+"</code> not built";\n'
