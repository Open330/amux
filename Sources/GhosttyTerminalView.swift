    ...
    var detail: String {
        ...
        case .realUserConfig:
            return String(
                ...
                defaultValue: "Loads your actual Ghostty and amux config files."
            )
        case .freshInstall:
            return String(
                localized: "debug.startupAppearance.profile.freshInstall.detail",
                defaultValue: "No user theme or terminal colors, so amux applies its managed default colors."
            )
        case .userThemePair:
            return String(
...
class GhosttyApp {
    ...
    private var keyboardInputSourceObserver: GhosttyKeyboardInputSourceObserver?
    ...
    private func initializeGhostty() {
        ...
        // AppKit's text-input notification is not reliable for every keyboard
        // toggle path, so the bridge also listens to the system TIS notification.
        keyboardInputSourceObserver = GhosttyKeyboardInputSourceObserver { [weak self] in
            guard let app = self?.app else { return }
