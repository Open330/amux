...
/// `amux.json` via ``CmuxSettings``, not `@AppStorage`, so the value applies to
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text(String(
                ...
                defaultValue: "New DEBUG amux windows open on the selected display. Shared across all tagged dev builds; applied at window creation."
            ))
