...
private func browserPresentExternalNavigationPrompt(
    ...
    alert.informativeText = String(
        ...
        defaultValue: "A web page in amux wants to open a link in another app. You can stay in the browser instead."
    )
...
private func browserPresentExternalNavigationFailure(
    ...
    alert.informativeText = String(
        ...
        defaultValue: "amux could not open this link. You can copy it and open it in another app."
    )
...
final class BrowserHistoryStore: ObservableObject {
    ...
    private func migrateLegacyTaggedHistoryFileIfNeeded(to targetURL: URL) {
        for legacyURL in Self.location()?.legacyHistoryFileURLs ?? [] {
            fileRepository.migrateLegacyFileIfNeeded(legacyURL: legacyURL, to: targetURL)
        }
    }
    ...
    var errorDescription: String? {
        ...
        case .missingDestinationProfile:
            return String(
                ...
                defaultValue: "The selected amux browser profile no longer exists. Pick a destination profile again."
            )
        case .profileCreationFailed(let name):
            return String(
                format: String(
                    localized: "browser.import.error.destinationCreateFailed",
                    defaultValue: "amux could not create the destination profile \"%@\"."
                ),
                name
            )
        }
    ...
    private func presentImportDialog(
        ...
        guard !browsers.isEmpty else {
            ...
            alert.informativeText = String(
                ...
                defaultValue: "amux could not find browser profiles to import from on this Mac."
            )
            alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK"))
            alert.runModal()
            return
    ...
    private final class ImportWizardWindowController: NSObject, @preconcurrency NSWindowDelegate {
        ...
        private func setupSourceProfilesContainer() {
            ...
            sourceProfilesHelpLabel.stringValue = String(
                ...
                defaultValue: "Select one or more profiles."
            )
        ...
        private func rebuildStep3DestinationUI() {
            ...
            if presentation.showsSeparateRows {
                destinationHelpLabel.stringValue = String(
                    ...
                    defaultValue: "Missing amux profiles are created when import starts."
                )
                destinationHelpLabel.isHidden = false
            } else if plan.entries.count > 1 {
                destinationHelpLabel.stringValue = String(
                    localized: "browser.import.destinationProfile.mergeHelp",
                    defaultValue: "All selected source profiles will be merged into the chosen amux browser profile."
                )
                destinationHelpLabel.isHidden = false
            } else {
