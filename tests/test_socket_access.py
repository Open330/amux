...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/amux DEV.app")
...
def _find_app():
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/cmux-{preferred_slug}/Build/Products/Debug/amux DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/cmux-{preferred_slug}/Build/Products/Debug/amux DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/amux DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/cmux-*/Build/Products/Debug/amux DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/cmux-*/Build/Products/Debug/amux DEV*.app"))
...
def _kill_cmux(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/amux DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "amux DEV"], capture_output=True)
    time.sleep(1.5)
...
def run_tests():
    ...
    app_path = _find_app()
    if not app_path:
        print("Error: Could not find amux DEV.app in DerivedData")
        return 1
