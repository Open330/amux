...
reject_text "$ROOT_DIR/scripts/build-signed-dmg.sh" 'ln -sfn amux "$BIN_DIR/cmux"'
require_text "$ROOT_DIR/scripts/verify-amux-release-dmg.sh" 'must not include the retired cmux CLI alias'
require_text "$ROOT_DIR/Packages/macOS/CmuxSettings/Sources/CmuxSettings/Stores/CmuxConfigLocation.swift" '.config/amux/amux.json'
require_text "$ROOT_DIR/Packages/macOS/CmuxSettings/Sources/CmuxSettings/Migration/AmuxPathMigration.swift" '.config/cmux'
require_text "$ROOT_DIR/Packages/macOS/CmuxSettings/Sources/CmuxSettings/Migration/AmuxPathMigration.swift" '.cmuxterm'
require_text "$ROOT_DIR/CLI/CMUXCLI+DocsSettings.swift" 'primarySettingsDisplayPath = "~/.config/amux/amux.json"'
require_text "$ROOT_DIR/CLI/CMUXCLI+DocsSettings.swift" 'aliases: ["configuration", "config", "amux-json"'
reject_text "$ROOT_DIR/Sources/CmuxConfig.swift" 'appendingPathComponent(".cmux")'
reject_text "$ROOT_DIR/Sources/DockSplitStore+Config.swift" 'appendingPathComponent(".cmux")'
require_text "$ROOT_DIR/Packages/macOS/CmuxRemoteSession/Sources/CmuxRemoteSession/Session/RemoteSessionCoordinator+RelayProvisioning.swift" 'mv -f "$wrapper_tmp" "$HOME/.amux/bin/amux"'
...
require_text "$ROOT_DIR/web/data/amux.schema.json" 'Compatibility key retained by amux; telemetry is disabled.'
