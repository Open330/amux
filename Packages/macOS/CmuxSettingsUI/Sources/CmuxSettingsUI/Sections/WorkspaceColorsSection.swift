...
public struct WorkspaceColorsSection: View {
    ...
    private var mainCard: some View {
        SettingsCard {
            ...
            SettingsCardNote(
                String(localized: "settings.workspaceColors.dictionaryNote", defaultValue: "Edit amux.json to add or remove named colors. \"Choose Custom Color...\" still adds local Custom N entries.")
            )

            let entries = effectivePaletteEntries(overrides: paletteModel.current)
            if entries.isEmpty {
                SettingsCardNote(
                    String(localized: "settings.workspaceColors.emptyPalette", defaultValue: "No palette entries. Add colors in amux.json or use \"Choose Custom Color...\" from a workspace context menu.")
                )
            } else {
