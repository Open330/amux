...
extension UpdateStateModel {
    ...
    public static func userFacingErrorMessage(for error: any Swift.Error) -> String {
        ...
        if let networkError = networkError(from: nsError) {
            switch networkError.code {
            case NSURLErrorNotConnectedToInternet:
                return String(localized: "update.error.noInternet.message", defaultValue: "amux can’t reach the update server. Check your internet connection and try again.")
            case NSURLErrorTimedOut:
            ...
            case NSURLErrorCannotConnectToHost:
                return String(localized: "update.error.serverUnreachable.message", defaultValue: "amux couldn’t connect to the update server. Check your connection or try again later.")
            case NSURLErrorNetworkConnectionLost:
        ...
        if nsError.domain == SUSparkleErrorDomain {
            switch nsError.code {
            case 2001:
                return String(localized: "update.error.feedDownload.message", defaultValue: "amux couldn't download the update feed. Check your connection and try again.")
            case 1000, 1002:
            ...
            case 1003, 1005:
                return String(localized: "update.error.permissionError.message", defaultValue: "Move amux into Applications and relaunch to enable updates.")
            case 4005, 4010:
                return String(localized: "update.error.installRecovery.message", defaultValue: "Move amux into Applications and relaunch to enable updates. If it’s already in Applications, restart your Mac and try again, or download the latest version below.")
            default:
                break
            }
        }
